        ...
        <div class="container_main">
            ...
            <div class="main">
                ...
                <button class="h-6 w-40 mb-8 bg-blue-100 rounded-md mt-2 ring-2 ring-blue-400 text-base no-underline"
                    type="button">
                    <a href="template_detail.php?id=<?= $id; ?>">詳細に戻る</a></button>


                <form action="?action=edit" method="POST">
                    <div class="flex mb-2">
                        <span class="my-auto text-center w-1/12">TO:</span><textarea name="to" id=""
                            class="w-11/12 h-40 border-2" charset="UTF-8"><?= $detail['mail_to'] ?></textarea>
                    </div>
                    <div class="flex mb-2">
                        <span class="my-auto text-center w-1/12">CC:</span><textarea name="cc" id=""
                            class="w-11/12 h-40 border-2" charset="UTF-8"><?= $detail['mail_cc'] ?></textarea>
                    </div>
                    <div class="flex mb-2">
                        <span class="my-auto text-center w-1/12">BCC:</span><textarea name="bcc" id=""
                            class="w-11/12 h-8 border-2" charset="UTF-8"><?= $detail['mail_bcc'] ?></textarea>
                    </div>
                    <div class="flex mb-2">
                        <span class="my-auto text-center w-1/12">タイトル:</span><textarea name="title" id=""
                            class="w-11/12 h-8 border-2" charset="UTF-8"><?= $detail['title'] ?></textarea>
                    </div>
                    <div class="flex mb-2">
                        <span class="my-auto text-center w-1/12">内容:</span><textarea name="content" id=""
                            class="w-11/12 h-96 border-2" charset="UTF-8"><?= $detail['content'] ?></textarea>
                    </div>
                    <div class="flex mb-2">
                    ...
                    <div class="flex justify-items-start my-4">
                        ...
                        <div class="w-11/12 h-full">
                            ...
                            <input class="mx-2" type="checkbox" name="tagbox[]" id="<?= $tag['id'] ?>"
                                <?= in_array($tag['name'], $col) ? "checked" : "" ?>></input>
